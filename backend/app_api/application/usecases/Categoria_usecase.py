...
class CategoriaUseCase:
    ...
    def create(self, nombre:str, descripcion:str | None)-> CategoriaEntity:
        if not all([nombre]):
            raise Exception("Todos los campos son obligatorios: nombre")
        try:
            obj = self.repository.save(CategoriaEntity(
                id=None,
                nombre=nombre,
    ...
    def update(self, id, **kwargs)-> CategoriaEntity:
        if not id:
