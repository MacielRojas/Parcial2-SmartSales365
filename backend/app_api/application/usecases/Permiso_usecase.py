from app_api.domain.entities.Permiso_entity import PermisoEntity

from app_api.application.repositories.Permiso_repository import PermisoRepository

class PermisoUseCase:
    def __init__(self, repository: PermisoRepository):
        self.repository = repository

    def get(self, **kwargs)->list[PermisoEntity] | PermisoEntity:
        try:
            if not id:
                obj = self.repository.get_all()
            else:
                obj = self.repository.get(**kwargs)
            if obj is None:
                raise Exception("No se encontraron registros")
            return obj
        except Exception as e:
            raise Exception(f"No se pudo obtener el registro: {e}")

    def create(self, nombre:str, descripcion:str | None)-> PermisoEntity:
        if not all([nombre]):
            raise Exception("Todos los campos son obligatorios: nombre")
        try:
            obj = self.repository.save(PermisoEntity(
                id=None,
                nombre=nombre,
                descripcion=descripcion))
            if not obj:
                raise Exception("No se pudo crear el registro")
            return obj
        except Exception as e:
            raise Exception(f"No se pudo crear el registro: {e}")

    def update(self, id, **kwargs)-> PermisoEntity:
        if not id:
            raise Exception("El id es obligatorio")
        try:
            obj = self.repository.get_by_id(id)
            if not obj:
                raise Exception("No se encontraron registros")
            data = obj.__dict__
            for key, value in kwargs.items():
                if key in data and key != 'id':
                    data[key] = value
            obj = PermisoEntity(**data)
            return self.repository.save(obj)
        except Exception as e:
            raise Exception(f"No se pudo actualizar el registro: {e}")

    def delete(self, id)->bool:
        if not id:
            raise Exception("El id es obligatorio")
        try:
            obj = self.repository.get_by_id(id)
            if not obj:
                raise Exception("No se encontraron registros")
            return self.repository.delete(id)
        except Exception as e:
            raise Exception(f"No se pudo eliminar el registro: {e}")

