...
class UserRolView(APIView):
    permission_classes = [AllowAny]
    def __init__(self):
        pass

    def get(self, request, id=None):
        obj_crud = UserRolUseCase(repository=DjangoUserRolRepository())
        try:
    ...
    def post(self, request):
        ...
        if isinstance(data, list):
            ...
            for idx, item in enumerate(data):
                try:
                    obj = obj_crud.create(user=item.get("user"),rol=item.get("rol"))
                    if obj:
        ...
        try:
            obj = obj_crud.create(user=data.get('user'), rol=data.get('rol'))
