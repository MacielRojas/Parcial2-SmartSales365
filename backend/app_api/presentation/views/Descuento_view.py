from app_api.application.usecases.Descuento_usecase import DescuentoUseCase
from app_api.infraestructure.repositories.djangoDescuento_repository import DjangoDescuentoRepository
...
class DescuentoView(APIView):
    ...
    def get(self, request, id=None):
        obj_crud = DescuentoUseCase(repository=DjangoDescuentoRepository())
        try:
    ...
    def post(self, request):
        ...
        if isinstance(data, list):
            ...
            for idx, item in enumerate(data):
                try:
                    obj = obj_crud.create(tipo=item.get("tipo"),producto=item.get("producto"),valor=item.get("valor"),fecha_inicio=item.get("fecha_inicio"),fecha_fin=item.get("fecha_fin"))
                    if obj:
        ...
        try:
            obj = obj_crud.create(tipo=data.get('tipo'), producto=data.get('producto'), valor=data.get('valor'), fecha_inicio=data.get('fecha_inicio'), fecha_fin=data.get('fecha_fin'))
