from app_api.application.usecases.MLmodel_usecase import MLmodelUseCase
...
from app_api.permission import EsAdmin
...
class MLmodelView(APIView):
    ...
    def get(self, request, id=None):
        obj_crud = MLmodelUseCase(repository=DjangoMLmodelRepository())
        try:
    ...
    def post(self, request):
        ...
        if isinstance(data, list):
            ...
            for idx, item in enumerate(data):
                try:
                    obj = obj_crud.create(name=item.get("name"),version=item.get("version"),model_data=item.get("model_data"),description=item.get("description"))
                    if obj:
        ...
        try:
            obj = obj_crud.create(name=data.get('name'), version=data.get('version'), model_data=data.get('model_data'), description=data.get('description'))
