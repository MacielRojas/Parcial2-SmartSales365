...
class CarritoView(APIView):
    ...
    def get(self, request, id=None):
        obj_crud = CarritoUseCase(repository=DjangoCarritoRepository())
        try:
    ...
    def post(self, request):
        ...
        if isinstance(data, list):
            ...
            for idx, item in enumerate(data):
                try:
                    obj = obj_crud.create(usuario=item.get("usuario"),total=item.get("total"),descuento=item.get("descuento"))
                    if obj:
        ...
        try:
            obj = obj_crud.create(usuario=data.get('usuario'), total=data.get('total'), descuento=data.get('descuento'))
