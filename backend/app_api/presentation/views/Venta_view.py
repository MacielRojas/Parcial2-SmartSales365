...
class VentaView(APIView):
    ...
    def get(self, request, id=None):
        obj_crud = VentaUseCase(repository=DjangoVentaRepository())
        try:
    ...
    def post(self, request):
        ...
        if isinstance(data, list):
            ...
            for idx, item in enumerate(data):
                try:
                    obj = obj_crud.create(carrito=item.get("carrito"),pago=item.get("pago"))
                    if obj:
        ...
        try:
            obj = obj_crud.create(carrito=data.get('carrito'), pago=data.get('pago'))
