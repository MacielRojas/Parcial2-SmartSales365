...
class PagoView(APIView):
    ...
    def get(self, request, id=None):
        obj_crud = PagoUseCase(repository=DjangoPagoRepository())
        try:
    ...
    def post(self, request):
        ...
        if isinstance(data, list):
            ...
            for idx, item in enumerate(data):
                try:
                    obj = obj_crud.create(monto=item.get("monto"),moneda=item.get("moneda"),estado=item.get("estado"),carrito=item.get("carrito"),payment_method_id=item.get("payment_method_id"),payment_intent=item.get("payment_intent"))
                    if obj:
        ...
        try:
            obj = obj_crud.create(monto=data.get('monto'), moneda=data.get('moneda'), estado=data.get('estado'), carrito=data.get('carrito'), payment_method_id=data.get('payment_method_id'), payment_intent=data.get('payment_intent'))
