...
class ProductoView(APIView):
    ...
    def get(self, request, id=None):
        obj_crud = ProductoUseCase(repository=DjangoProductoRepository())
        try:
    ...
    def post(self, request):
        ...
        if isinstance(data, list):
            ...
            for idx, item in enumerate(data):
                try:
                    obj = obj_crud.create(nombre=item.get("nombre"),precio=item.get("precio"),stock=item.get("stock"),codigo=item.get("codigo"),marca=item.get("marca"),categoria=item.get("categoria"))
                    if obj:
        ...
        try:
            obj = obj_crud.create(nombre=data.get('nombre'), precio=data.get('precio'), stock=data.get('stock'), codigo=data.get('codigo'), marca=data.get('marca'), categoria=data.get('categoria'))
