from app_api.application.usecases.Mantenimiento_usecase import MantenimientoUseCase
from app_api.infraestructure.repositories.djangoMantenimiento_repository import DjangoMantenimientoRepository
...
class MantenimientoView(APIView):
    ...
    def get(self, request, id=None):
        obj_crud = MantenimientoUseCase(repository=DjangoMantenimientoRepository())
        try:
    ...
    def post(self, request):
        ...
        if isinstance(data, list):
            ...
            for idx, item in enumerate(data):
                try:
                    obj = obj_crud.create(producto=item.get("producto"),precio=item.get("precio"),fecha_programada=item.get("fecha_programada"),estado=item.get("estado"),usuario=item.get("usuario"))
                    if obj:
        ...
        try:
            obj = obj_crud.create(producto=data.get('producto'), precio=data.get('precio'), fecha_programada=data.get('fecha_programada'), estado=data.get('estado'), usuario=data.get('usuario'))
